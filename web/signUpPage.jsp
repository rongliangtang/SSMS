...
<div class="box">
    ...
    <div class="jump">
        <a href="loginPage.jsp">已经有账号？快去登录</a>
    </div>
    <button type="button" class="btn-box" id="signUp">注&nbsp&nbsp&nbsp册</button>
...
<script>
    $("#signUp").click(function () {
        ...
        if (username && password) {
            $.ajax({
                url: "http://localhost:8080/signUpCheck",
                type: "post",
                data: {
                    username: username,
                    password: password
                ...
                success: function (result) {
                    ...
                    if (flag == true) {
                        //验证成功
                        $.growl.notice({title: "提示", message: "注册成功!"});
                        setTimeout(function(){
                            window.location.href = "http://localhost:8080/mainPage.jsp";
                        },1000)
                    } else {
                        //验证失败
                        $.growl.error({title: "提示", message: "用户名已经被占用了!"});
                    }
                }
            })
        } else {
